const scrollFrames = {
  ...
  getIntersection: function (el) {
    ...
    if (typeof el == "object") {
      ...
      if (typeof el.getBoundingClientRect == "function") {
        ...
        response =
          (window_scroll_top + window_height - el_offset_top) /
          (window_height + el_height);
      }
    }
        ...
        if (anim_id.length > 0 && anim_id !== undefined) {
          ...
          if (anim_url_mask !== undefined && anim_json_url === undefined) {
            ...
            if (anim_mask_array.length == 3) {
              ...
              for (
                let frame_index = anim_mask_i_start;
                frame_index <= anim_mask_i_end;
                frame_index++
              ) {
                json_from_mask.push({
  ...
  build: function (json, el_anim, anim_id, anim_keyframes) {
        ...
        if (anim_keyframes.indexOf(":") > 0) {
          ...
          fromToArray.forEach(function (keypoint, keyPointIndex) {
            ...
            if (keyPointIndex < fromToArray.length - 1) {
              ...
              const positionNext =
                parseFloat(fromToArray[keyPointIndex + 1].split(":")[0]) / 100;

              if (
                scroll_line >= positionCurrent &&
                scroll_line <= positionNext
              ) {
    ...
    scrollFrames["anims"][anim_id]["el"] = document.querySelector(
      '[scroll-frames="' + anim_id + '"]'
    );
    // If optional detector is set
    if (scrollFrames["anims"][anim_id]["el"] !== null) {
      const optional_detector_id =
        scrollFrames["anims"][anim_id]["el"].dataset.detector;
    ...
    el_anim.style.backgroundImage =
      scrollFrames["anims"][anim_id]["backgroundImage"];
  ...
  frame: function () {
    ...
    Object.keys(scrollFrames.anims).forEach(function (anim_id, anim_index) {
      ...
      if (
        scrollFrames["anims"][anim_id]["ready"] &&
        scrollFrames["anims"][anim_id]["el"] !== null
      ) {
        let current_scroll_line = 0;
        if (
          scrollFrames["anims"][anim_id]["el_detector"] !== null &&
          scrollFrames["anims"][anim_id]["el_detector"] !== undefined
        ) {
          current_scroll_line = scrollFrames.getIntersection(
            scrollFrames["anims"][anim_id]["el_detector"]
          );
        } else {
          current_scroll_line = scrollFrames.getIntersection(
            scrollFrames["anims"][anim_id]["el"]
          );
        }
        const frameIndex =
          scrollFrames["anims"][anim_id]["transfer"](current_scroll_line);
        const index = Math.round(
          frameIndex *
            (scrollFrames["anims"][anim_id]["backgroundSizes"].length - 1)
        );
        // console.log(anim_id, current_scroll_line, frameIndex);
        // Apply proper background-size property
        scrollFrames["anims"][anim_id]["el"]["style"]["background-size"] =
          scrollFrames["anims"][anim_id]["backgroundSizes"][index];
      }
    });
  },
};
